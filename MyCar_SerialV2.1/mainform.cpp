    ...
    ui(new Ui::mainform)
{
    ...
    setAttribute(Qt::WA_QuitOnClose,true);//主窗口关闭其余子窗口一起关闭
    m_number_send = 0;
    m_number_recive =0;
    showCountNumber();
...
{
    if(m_number_recive >10000)
    {
        on_pushButton_clicked();//定期清除接收区
        m_number_recive = 0;
    }
...
/********************************* End user code. *********************************************/
        ...
        {
            ui->textBrowser->insertHtml(toBlueText(QString::fromLocal8Bit(data)));//insertHtml
        }
...
void mainform::Leather_Data_Send(quint16 posionX, quint16 posionY, quint8 flag)
{
    QByteArray array;
